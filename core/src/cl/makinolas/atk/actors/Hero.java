...
import cl.makinolas.atk.actors.heroState.ParalizedState;
...
public class Hero extends Monsters implements IHero {
			...
			if (friends.length == 0) {
				...
				for (int i = 0; i < friends.length; i++) {
					addAllie(MonsterFactory.getInstance().getHeroFriend(friends[i].name, friends[i].level,
							friends[i].exp, friends[i].individualValue, friends[i].ev1, friends[i].ev2));
				}
			}
		} else {
			addAllie(MonsterFactory.getInstance().getHeroFriend("Kakuna", 6));
	...
	/*
	 * changeAlliesTeam debe intercambiar durante la partida un pokemon dentro
	 * del equipo por uno de los atrapados (pero que no son utilizables en la
	 * version final del juego), es solo un test, y posiblemente sirva para
	 * estudiar mejor ciertos experimentos en las otras fases del proyecto (como
  ...
  public void setMovablePLatformSpeed(float vX, float vY) {
	...
	/*
	 * @Deprecated private float getImpulse(float impulse) { return
	 * getBody().getMass()*impulse; // El 12 se busco por testing. }
	 */
	...
	public void landedPlatform(WorldManifold worldManifold, Platform platform) {
		...
		for (int i = 0; i < worldManifold.getNumberOfContactPoints(); i++) {
			if (worldManifold.getPoints()[i].y < myBody.getPosition().y
					&& (worldManifold.getNormal().y > 0.95 || worldManifold.getNormal().y < -0.95)) {
				isJumping = false;
  ...
  public void setAnimation() {
	...
	public void endPlatformInteraction(Platform platform, WorldManifold worldManifold) {
		platform.endHeroInteraction(this, worldManifold);
	}
	...
	public void checkHorizontal() {
		if (!pressingLeft && !pressingRight) {
			...
			moveHorizontal(actualFriend.getVex());
	...
	public void attackPrimary() {
		actualFriend.getState().attackPrimary(((AbstractStage) getStage()), actualFriend, myWorld, myBody.getPosition(),
				isFacingRight, this);
	}
	...
	public void throwBall(Ball.BallType type) {
		mplayer.playthrow();
		BallActor ball = new BallActor(type, myWorld,
				myBody.getPosition().x
						+ ((isFacingRight) ? 0.6f : -0.6f) * getActualFriend().getWidth() / GameConstants.WORLD_FACTOR,
  ...
  @Override
	...
	@Override
	public void sing() {
		actualFriend.setState(new SleepState(actualFriend.getState()));
		this.addState(manaReduce = new ManaReduceState(this, 15, actualFriend.getState().getAttack()), 100);
		this.changeAnimation(hurtAnimation);
	}

	@Override
	public void unSing() {
		actualFriend.setState(new StandartState(actualFriend.getState()));
	...
	public void paraliza3() {
		vx = 0;
		actualFriend.setVex(3);
		actualFriend.setState(new ParalizedState(actualFriend.getState()));
	}

	@Override
	public void desparaliza3() {
		vx = 0;
		actualFriend.setVex(7);
		actualFriend.setState(new StandartState(actualFriend.getState()));
	}
